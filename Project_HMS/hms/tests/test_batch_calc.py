...
def test_async_avg_age(event_loop):
    patients = [
        {"id": 1, "name": "Bob", "age": 40, "disease": "Flu"},
        {"id": 2, "name": "Sumit", "age": 30, "disease": "Diabetes"},
        {"id": 3, "name": "Dravid", "age": 50, "disease": "Asthma"},
    ]
    avg_age = event_loop.run_until_complete(
        batch_calc.calculate_average_age_async(patients, batch_size=2)
    )
    assert round(avg_age, 2) == round((40 + 30 + 50) / 3, 2)
